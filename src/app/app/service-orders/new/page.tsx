...
import {
  ...
  SERVICE_ORDER_WIZARD_STEP_FIELDS,
  serviceOrderWizardSchema,
  type ServiceOrderItemValues,
  type ServiceOrderWizardValues,
...
function NovaOsWizard() {
  ...
  const form = useForm<ServiceOrderWizardValues>({
    resolver: zodResolver(serviceOrderWizardSchema),
    defaultValues: {
      client_id: null,
    ...
    try {
      ...
      if (values.lock_type !== "none" && values.lock_value?.trim()) {
        try {
          ...
          toast.warning("OS criada, mas o dado de bloqueio não foi salvo com segurança", {
            description:
  ...
  async function handleNext() {
    ...
    const fields = SERVICE_ORDER_WIZARD_STEP_FIELDS[stepIndex] ?? []
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-6">
      <PageHeader title="Nova Ordem de Serviço" description="Registre a entrada do aparelho, passo a passo." />
      ...
      <Stepper steps={STEPS} onStepChange={setStepIndex}>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                {(
                  ...
                  <div key={item.name} className="border-border flex items-center justify-between rounded-xl border p-3">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="border-warning-bg bg-warning-bg text-warning flex items-start gap-2 rounded-xl border p-3 text-xs">
                ...
                <span>
                  ...
                  técnico atribuído ou pelo dono da assistência. É apagado automaticamente 7 dias depois da OS ser
                  marcada como paga.
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5 sm:max-w-40">
                ...
                <Input
                  type="number"
                  min={0}
                  value={form.watch("warranty_days")}
                  onChange={(e) => form.setValue("warranty_days", Number(e.target.value) || 0)}
                />
              </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-foreground font-medium">
                    {lockType === "none" ? "Sem bloqueio" : `${lockType === "pattern" ? "Padrão" : "Senha"}: ${form.watch("lock_value") || "(em branco)"}`}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-foreground font-bold">
                    {(
                      items.reduce((sum, item) => sum + item.quantity * item.unit_price_cents, 0) / 100
        ...
        <StepperFooter onNext={handleNext} submitting={createServiceOrder.isPending} finishLabel="Confirme e Criar" />
      </Stepper>
    </div>
  )
}
